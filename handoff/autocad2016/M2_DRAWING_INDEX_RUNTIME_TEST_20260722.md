...
> **停止：本节的候选身份已于 2026-07-26 失效，不要按它执行实机测试。**
>
> M2、M3 和 M4 线已于 2026-07-26 全部吸收进 `main`（`77a6cdf`）。汇合改变了
> Host.2016 的源码，因此下面这组哈希描述的是一个**不会出货的构建**。按它加载只会
> 得到一份与主线无关的旧二进制，测出来的结论无法绑定到任何将要发布的候选。
>
> 执行 M2 实机测试前必须先在 `main` 上重新构建候选，并把本节整体替换为新的候选目录
> 与哈希。构建入口为 `scripts/verify-all-gates.ps1`（必过门禁套件）与 R20.1 候选构建
> 门禁；当前 `main` 的 Phase 2 尚未全绿，详见 `CURRENT_STATE.md` 的
> 「主线汇合与门禁状态」一节。
>
> 以下内容保留为历史记录。第 1 节起的测试步骤本身仍然适用。
