...
5. Palette 125% DPI。
6. Palette 150% DPI。
7. 文档关闭和 AutoCAD 正常退出生命周期及残留检查。
8. 最后才进入预览、拒绝、一次允许、锁内重校验和单事务写入实测。

## 下一步顺序

1. 核心 Agent MVP 已在 `7f10d60` 单独提交，并保留旧候选停止残留边界。
2. `0.3.2.0` 停止修复已完成自动化门禁、真实 R20.1 双构建和新候选冻结；等待最小 AutoCAD 两轮启停
   复验。通过后写入脱敏 evidence 并单独提交生命周期修复。
3. 生命周期提交后冻结 CadContextJson v2 对象覆盖契约，实现第一批高频对象和显式未知
   对象占位，完成 net45/net8、Host 和混合选择 Specs 后再请求实机验证并单独提交。
