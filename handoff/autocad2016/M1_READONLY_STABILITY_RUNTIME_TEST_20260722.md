...
尚未执行：第 5 节（新建对话）、第 6 节（只清 CAD 上下文）、第 7 节（清除全部）、
第 8 节（图 A/图 B 隔离）、第 9 节（取消与 busy）、第 10 节（Palette Reset 与停止重启）、
第 11 节（正常退出清理）、第 12 节（125% 与 150% DPI）、第 13 节（故障观察）。

在以上各节全部回报之前，M1 实机验收不算完成。
