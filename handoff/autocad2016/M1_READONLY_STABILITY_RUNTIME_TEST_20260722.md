...
> ## ⚠ 部分撤回（2026-07-26 当日）
>
> 用户当日晚些时候澄清了实际执行范围。下方记录按此更正——**保留真实观察到的项，
> 撤回其余全部**。
>
> **实际观察到、予以保留的（第 3 节，三项）：**
>
> - `NETLOAD` 成功加载 `Codex.AutoCAD.Host.2016.dll`
> - `Module version` 显示 `0.3.3.0`
> - Palette 上 CAD write 与 plugin-initiated save 均显示 disabled
>
> **未执行、全部撤回的：**
>
> - **Agent 连接从未启动**（`CODEX16AGENTSTART`）。第 3 节的两轮问答、第 4 节整节，
>   以及第 5–12 节全部依赖 AgentHost 在线，因此**一项都没有实际执行**。
> - `DBMOD` 前后比对：用户未提及，视为未确认，不得引用。
> - 据此得出的"M1 实机验收完成"结论。
>
> 与之相符的技术证据（当日同时发现）：
>
> - `CODEX16AGENTSTART` 实际走 `bootstrap-serve`；同一台机器上该路径**稳定 10 秒超时**，
>   而 `bootstrap-doctor` 168 毫秒通过。M1 候选与汇合后候选的 AgentHost 表现一致。
> - 受保护审计根下**最新的会话审计记录停在 2026-07-23**，此后没有任何成功服务的痕迹。
>
> 也就是说：若这些小节当日真的执行过，AgentHost 应当无法启动，测试不可能通过。
> 撤回与证据一致。
>
> **原文保留在下方，不删除。**删掉会让"这里曾经写过什么、为什么错"一并消失，而那
> 正是以后判断类似记录是否可信时唯一能依靠的东西。重新执行前，M1 实机验收状态为
> **未验证**。
>
> 重新执行的前置条件：先修复 `bootstrap-serve`。相关门禁盲区见同日记录——
> bootstrap 门禁只覆盖 `bootstrap-doctor`，从未覆盖生产使用的 `bootstrap-serve`。

### 2026-07-26 冒烟（第 3、4 节）〔部分撤回：仅前三项属实〕

用户加载当前候选后回报：

| 检查项 | 结果 |
| --- | --- |
| `NETLOAD` 加载成功 | **通过（实际观察）** |
| `Module version` = `0.3.3.0` | **通过（实际观察）** |
| CAD write / plugin-initiated save 显示 disabled | **通过（实际观察）** |
| `DBMOD` 前后未变化 | ~~通过~~ **撤回：未确认** |
| 两轮问答正常返回 | ~~通过~~ **撤回：Agent 连接从未启动** |

结论（更正后）：候选可在真机 `NETLOAD`，并正确显示模块版本与 CAD 写入/插件保存的禁用
状态。**没有任何问答闭环证据**——`CODEX16AGENTSTART` 未执行。
...
**当前状态：M1 实机验收未验证。**第 3–12 节没有任何一节有可采信的实机结果。

代码侧的吸收已经完成（`codex/m1-integration` 已进入 `main`），但**吸收不等于实机验收**。
在 `bootstrap-serve` 修复并重新执行第 3–12 节之前，不得把 M1 称为已验收，也不得在任何
evidence、交接文档或提交信息中出现"M1 实机验收完成"。
