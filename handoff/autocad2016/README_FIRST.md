...
- 已在目标机采集到原版 AutoCAD 2016 R20.1 x64 及其 Autodesk 签名托管程序集，`AcMgd`/`AcDbMgd` 程序集版本均为 `20.1.0.0`。当前 schema v5 采集器同时识别 `AcadLocation`、`InstallLocation`、`Location`，双 PowerShell 真实只读采集均得到安装 `1`、可构建安装 `1`，注册表读取失败 `0`；2026-07-20 的双 Shell `24/24` 跟进回归又实际注入并覆盖四类注册表读取失败分支。
...
| 范围 | 当前状态 | 证据边界 |
| --- | --- | --- |
| AutoCAD 2016 环境采集 | 已通过 | 当前 schema v5；目标安装 `1`、可构建安装 `1`；支持 `AcadLocation`/`InstallLocation`/`Location`，历史发现自测 `10/10`，跟进失败注入回归 PS7/PS5.1 均 `24/24`，真实采集读取失败 `0`；历史 schema v4 证据继续保留 |
...
1. 已采集并锁定目标机 AutoCAD 2016 R20.1、x64、`acad.exe` 文件版本 `R20.1.49.0.0`、Autodesk 有效签名和托管 API 版本；schema v5 又补齐 `Location` 注册表值、脱敏读取失败计数及双 PowerShell 历史 `10/10` 发现回归，后续双 Shell `24/24` 失败注入回归覆盖 probe、根键、子键和属性读取失败。
...
- `handoff/autocad2016/evidence/environment-collector-failure-regression-20260720.json`：以 `a011dd1` 为基线的双 PowerShell `24/24` 失败注入回归、目标机双运行时只读复采及原始本地报告哈希绑定；原始报告不提交。
