...
- 每个可验证阶段通过后单独提交 Git。
- 任意 `DOTNET_CLI_HOME` 隔离构建必须同时设置
  `DOTNET_ADD_GLOBAL_TOOLS_TO_PATH=0`；所有清空环境后启动的 `dotnet` 子进程也必须显式
  传递该值。运行长门禁前后必须检查用户 PATH 无项目临时 `.dotnet/tools` 项且哈希不变。
  不得通过 `setx PATH` 或 User/Machine 级环境变量修改 PATH。
...
**M1 已完成**（代码、自动化、候选冻结、实机矩阵，并已于 2026-07-26 受控吸收进 `main`）。

当前候选：

- Host 版本：`0.3.3.0`。
- 集成分支：`codex/m1-integration`，从 `main@9edc83e` 受控吸收 10 个提交；无
  Host.2025/Kimi/M4 夹带。
- 候选：`autocad2016-m1-readonly-v033-e6701a77-8e6b26fd-7e69cb73`。
- Host SHA-256：
  `E6701A771D17EC3EC8B2CA7DA78B553E27897639DC48B3BC0435F07249C9B5F6`
  —— 与 2026-07-22 初次冻结逐字节一致，R20.1/net45 构建可复现。
- AgentHost SHA-256：
  `8E6B26FD7B20925A1CE53CAB0DBEE093C58B9AF0935219DF75FC8A7CB5C4FA2A`。
  framework-dependent 的 net8 发布哈希随 SDK 版本变化，历史上有三个值
  （`7A3ABCEA` 初版 → `4B602965` 集成冻结 → `8E6B26FD` 当前，与 M2/M3 候选一致）；
  这是可解释的发布差异，不是篡改信号，但 evidence 必须绑定当前值。
- manifest SHA-256：
...
- 自动化：Host MVP `41/41`；PowerShell 7 与 Windows PowerShell 5.1 均为 Phase 2
  `276/276`；R20.1/net45/x64 双构建位级一致、候选 AgentHost doctor、diff 和敏感信息
  扫描通过。
- 实机入口与结果：`M1_READONLY_STABILITY_RUNTIME_TEST_20260722.md` 第 15 节。
...
- [x] 取消、重复取消和活动回合期间新建/清除返回受限状态。
- [x] v2 上下文存在时 Palette Reset 后仍保留。
- [x] 不先 STOP，正常退出 AutoCAD 后残留清理。
- [x] 125% 和 150% DPI。
- [ ] 启动失败、Bridge 断线和超时按设计只在故障自然发生时记录，不构成阻塞项。

回报为分块整体通过，未逐项拆分：正常退出后的残留进程具体计数、`busy` 的确切错误码
取值、各 DPI 下按钮布局的逐项目视检查都没有单独数值记录。引用这些细节时必须重新
逐项回报，不得从"整体通过"推断。
...
> 汇合说明（2026-07-26）：以下同时保留各条线的原始记录。汇合前各线分别冻结过候选，
> 因此可能出现多组候选哈希——**它们全部早于本次汇合，均已失效，不得用于实机**；
> 汇合后必须重新构建候选并重新绑定 evidence。架构与语义描述仍然有效。

- [x] 保留 CadContextJson v2 作为兼容选择快照。
- [x] 新增版本化 DrawingIndex 和 CadQuery 契约。
- [x] 支持选择集、当前空间、模型空间、布局和整张图纸扫描范围。
- [x] AutoCAD API 读取在合法文档线程中分片；深拷贝后的统计、哈希和序列化可后台执行。
- [x] 支持进度、取消、超时和资源预算。
- [x] 建立类型、图层、空间、块、包围盒和对象数量索引。
- [x] 支持安全游标分页和按需查询，不一次发送整图 JSON。
- [x] 为 Codex 提供按类型、图层、块、范围、文字和对象 ID 的只读查询工具。
- [x] 大选择集发布总数、摘要、分页引用和完整性，不因数量直接失败。
- [x] 文档修改、撤销、切换或关闭后使旧索引失效。
- [x] 未知和代理对象形成受限占位，不中断整图扫描。
- [x] 建立并冻结 1,000、10,000、50,000 对象的脱敏基准图。
- [x] 冻结 UI 卡顿、总扫描时间、内存和 IPC 大小预算的自动化 evidence。
- [x] 超出预算返回 partial/limited，不崩溃、不假装完整。
- [ ] M2 候选 `v0.4.0` 完成精确 AutoCAD 实机扫描、分页和 DBMOD 矩阵后受控集成。
...
- [x] 提供中文对象名称、创建方法和脱敏示例测试图。
- [x] Palette/索引显示不支持对象的实际类型和数量。
- [x] 完善块属性、动态块、嵌套块、布局和空间信息的受限协议边界。
- [x] Xref 只公开安全元数据，不公开外部真实路径。
- [x] 完善 Dimension、Hatch、Leader、MLeader、Table 的 R20.1 API/字段边界。
- [x] 长文字、复杂 Hatch、Table、Spline 受限但不拖垮整体。
- [x] M3 自动化候选通过 Phase 2 `310/310`、benchmark `6/6` 和 R20.1 双 Shell API stage。
- [ ] 逐类实机验证现有 19 种强类型对象。
- [ ] 增加并验证 Region、Solid、Mesh、Surface、Image/Underlay 等高价值受限读取。
- [ ] 垂直产品代理对象至少提供类型、图层、范围和占位信息。
- [ ] 每类对象的实机字段 evidence 与 M3 精确候选哈希绑定后受控集成。
...
- M4.1 代码与自动化已收口（提交 `8418962`）：本机 Codex 绝对路径、工作目录、启动/关闭
  ...
  思考强度配置，以及机器策略 > 管理员 > 用户三层合并。低优先级层只能收窄白名单不能扩大，
  被高层锁定的项低层不得改动；未知、损坏、旧版本、缺省缺失、白名单为空或超限、默认值
  越界一律 fail-closed 且不返回部分策略。`CodexAgentRuntime` 三处模型字符串直接穿透已改走
  唯一出站边界，未配置策略时仍拒绝危险形态，返回值即实际下发值；`AgentHostPolicyStore`
  从固定位置加载并已接入生产 `bootstrap-serve`，三层皆缺失保留兼容行为，任一层存在却
  不可用则拒绝启动。规格由 `421` 增至 `454`。
  尚缺：企业策略分发与组策略锁定的真实验证；允许版本、请求超时、日志和资源策略仍分散在
  各自模块，未收拢为与模型/思考强度同构的统一配置层。
...
- [x] M4.15.4 补齐交接入口：它此前是 M4.15 中唯一没有任何交接文件的子项。新增
  ...
  执行步骤、预期稳定错误码、系统事件通道和证据表。**本项没有可先做的自动化部分**：分类
  映射与脱敏边界属于 M4.15.1，M4.15.4 只验证真实拦截是否确实落进那些错误码。
- [ ] M4.15.4 实机执行本身仍未开始，且需要用户在具备相应企业策略的机器上进行；A/B/D/E
  四个矩阵必须有结果，C/F 允许写明原因跳过。另需实机回答：当前用户身份下的普通错误 `5`
  归入 `ProcessStartBlocked`，会让纯 NTFS ACL 拒绝也显示“请让管理员检查 AppLocker、WDAC……”，
  是否构成误导；结论出来前不得凭猜测调整分类。
