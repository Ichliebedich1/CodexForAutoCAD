...
目的：区分活动线、已有提交的历史线、待复核候选和包含未提交内容的 Worktree。M0
自动化集成和候选冻结已完成；本文只建立 `main` 前进后的删除前置条件，不执行删除。
...
| `codex/m0-baseline` | `c96e9a3` 加证据/文档收口 | M0 候选已冻结 | 保留至进入 main 并打标签 |
