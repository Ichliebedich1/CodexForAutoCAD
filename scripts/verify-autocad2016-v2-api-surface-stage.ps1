...
$evidenceDir = if ([string]::IsNullOrWhiteSpace($EvidenceDirectory)) {
    ...
    [IO.Path]::GetFullPath($(if ([IO.Path]::IsPathRooted($EvidenceDirectory)) {
        $EvidenceDirectory
    }
    else {
...
if ($evidenceDir -cne $defaultEvidenceDir -and
    -not $evidenceDir.StartsWith(
        $artifactsPrefix, [StringComparison]::OrdinalIgnoreCase)) {
...
$historicalEvidencePath = Join-Path $defaultEvidenceDir "v2-api-surface-probe-verification.json"
if (-not (Test-Path -LiteralPath $historicalEvidencePath -PathType Leaf)) {
    throw "Historical evidence file is missing: $historicalEvidencePath"
}
$historicalEvidenceSha256Before = Get-Sha256 -Path $historicalEvidencePath
...
$historicalEvidenceSha256After = Get-Sha256 -Path $historicalEvidencePath
if ($historicalEvidenceSha256Before -cne $historicalEvidenceSha256After) {
    throw "Historical evidence changed during verification."
}
...
$crossShellEvidence = [ordered]@{
    ...
    autoCadProcessObservation = "same-pid-set-before-and-after; transient-process-start-not-observable"
...
try {
    foreach ($copy in @(
        [pscustomobject]@{ Source = $ps7EvidencePath; Destination = $ps7FinalPath },
        [pscustomobject]@{ Source = $ps51EvidencePath; Destination = $ps51FinalPath },
        [pscustomobject]@{ Source = $crossShellStagedPath; Destination = $crossShellEvidencePath }
    )) {
        Copy-Item -LiteralPath $copy.Source -Destination $copy.Destination -ErrorAction Stop
        $createdFinalPaths.Add($copy.Destination)
    }
}
catch {
    foreach ($createdPath in $createdFinalPaths) {
