...
var specs = new (string Name, Func<Task> Run)[]
{
    ...
    ("AgentHost会话Codex状态目录使用私有ACL且正常清理",
        AgentHostStorageSpecs.WorkspaceCodexStateUsesPrivateAclAndCleansNormally),
    ("AgentHost凭据引用创建隔离状态且不暴露测试令牌",
        AgentHostStorageSpecs.CodexSessionIsolationUsesApprovedCredentialOnly),
    ("AgentHost未配置凭据引用时保持兼容模式",
        AgentHostStorageSpecs.CodexSessionIsolationKeepsLegacyModeWhenUnconfigured),
    ("AgentHost无效凭据引用在创建状态前fail-closed",
        AgentHostStorageSpecs.CodexSessionIsolationRejectsInvalidCredentialReference),
    ("AgentHost凭据读取故障使用脱敏稳定错误",
        AgentHostStorageSpecs.CodexSessionIsolationSanitizesCredentialReaderFailure),
